...
We have now successfully removed missing values from the `wealth` variable which will make working with the variable a lot easier. We produce a few simple plots to describe our data which would require a lot more code had we not removed missing values.
...
lines(x = rep(mean(eu$wealth),10), y = seq(0, 4e-05, length.out = 10), col = "red", lwd = 2)
lines(x = rep(median(eu$wealth),10), y = seq(0, 4e-05, length.out = 10), col = "green", lwd = 2)
legend("topright", c("Mean", "Median"),  col = c("red", "green"), lwd = 2, bty = "n")
```
...
Let's plot over-time development of `wealth` in Essex. On the x-axis, we plot time and on the y-axis, we plot `wealth`.

```{r }
plot(
  x = eu$year[eu$region_name=="Essex"],
  y = eu$wealth[eu$region_name=="Essex"],
  xlab = "",
  ylab = "Wealth in Euro",
  main = "GDP per person in Essex adjusted by prices",
  bty = "n",
  col = "darkgrey",
  pch = 16,
  cex = 1.5
)
lines(x = eu$year[eu$region_name=="Essex"],
      y = eu$wealth[eu$region_name=="Essex"],
      lwd = 2,
      col = "darkgrey")
```
